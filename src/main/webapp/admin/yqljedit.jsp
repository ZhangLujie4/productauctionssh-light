...
<body>
		...
		Yqlj yqlj = (Yqlj)dao.findById(id,"Yqlj");
		 %>
           	<form name="usersform" method="post" action="yqljEdit.action?id=<%=id %>" id="usersForm">
					<table class="grid" cellspacing="1" width="100%" >
						<tr>
							...
							<td width="35%">
								<input name="name" id="name"  validate="{required:true,messages:{required:'请输入标题'}}" 
									class="input-txt" type="text" value="<%=yqlj.getName() %>"/><div id="errmsg" style="display: none;font-weight: "></div>
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<input name="url" id="url"  validate="{required:true,messages:{required:'友情链接地址'}}" 
									class="input-txt" type="text" value="<%=yqlj.getUrl() %>"/>
							</td>
						</tr>
