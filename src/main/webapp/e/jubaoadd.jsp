...
ArrayList<Goods> list = (ArrayList<Goods>)dao.findByHql("from Goods where status='正在拍卖' and memberid!='"+member.getId()+"' and delstatus='0'");
 %>
	...
	<div class="wrap" style="height: 850px;">
		...
		<div class="main" >
			...
			<div class="main-content" style="height: auto;">
               <div >
				<form name="huiyuanform" method="post" action="jubaoAdd.action?memberid=<%=member.getId() %>"
					id="huiyuanForm">
					<table class="grid" cellspacing="1" width="100%" border="0">
						...
						<tr>
							...
							<td>
							<select name="goodsid" id="goodsid" class="input-txt" style="width:258px;height:30px;" required>
							<option value="">请选择</option>
							<%for(Goods g:list){%>
							<option value="<%=g.getId() %>"><%=g.getGoodsname() %></option>
							<%} %>
							</select>
							</td>
